...
//                successLicenseImg.sd_setImageWithURL(NSURL.init(string: SHOW_IMAGE_HEADER+(companyInfo?.license)!))
                successLicenseImg.sd_setImageWithURL(NSURL.init(string: SHOW_IMAGE_HEADER+(companyInfo?.license)!), placeholderImage: UIImage.init(named: "1.png"))
                successView.frame.size = CGSizeMake(WIDTH, HEIGHT)
                self.view.addSubview(successView)
            }else if type == 4 {
                failView.frame.size = CGSizeMake(WIDTH, HEIGHT)
    ...
    override func viewDidLoad() {
        ...
        for myView in viewArr {
            ...
            if myView.restorationIdentifier == "auth" {
                ...
                reCertifyBtn?.setTitleColor(COLOR, forState: .Normal)
...
//        let dateStr = dateFormatter.stringFromDate(NSDate())
        ...
        HSMineHelper().updataCompanyCertifyWith(self.nameField.text!, companyinfo: self.descriptionField.text!, create_time: dateStr, phone: self.telField.text!, email: self.mailField.text!, linkman: self.contactField.text!, license: self.imageUrl!, handle: { (success, response) in
            ...
            if success {
                ...
                let doneAction = UIAlertAction(title: "确定", style: .Cancel, handler: { (action) in
                    ...
                    self.waitView.frame.size = CGSizeMake(WIDTH, HEIGHT)
    ...
    // MARK: 修改认证信息
    @IBAction func changeCompanyInfo(sender: UIButton) {
        ...
        let replyAction = UIAlertAction(title: "确认", style: .Default, handler: { (action) in
            ...
            self.authView.frame.size = CGSizeMake(WIDTH, HEIGHT)
    ...
    // MARK: 重新认证
    @IBAction func reCertify(sender: UIButton) {
