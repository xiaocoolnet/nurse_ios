//
//  GMyExaminationViewController.swift
//  HSHW_2016
//
...
class GMyExaminationViewController: UIViewController,UIScrollViewDelegate {
...
//        }
    ...
    func AnswerView_2() {
        ...
        if self.dataSource.count != 0{
            ...
            for i in 0 ..< examInfo.answers_2.count {
    ...
    func createScrollerView() {
        if self.type == 3 {
            createScrollerView_2()
        }else{
            createScrollerView_1()
        }
    }
    
    func createScrollerView_1() {
        ...
        scrollView.contentOffset = CGPoint(x: CGFloat( a )  * WIDTH, y: 0)
    ...
    func createScrollerView_2() {
        ...
        for  i in 0 ..< self.dataSource.count {
            ...
            let heightArray = NSMutableArray()
            for j in 0 ..< examInfo.answers_2.count {
                var string = ""
                let btn = UIButton()
                ...
                var AllHeight = CGFloat()
                AllHeight = CGFloat(heightArray.lastObject as! NSNumber)
                contentScrollView.contentSize = CGSize(width: WIDTH-20, height: AllHeight+10*CGFloat(examInfo.answers_2.count-1))
            }
...
//        time.tag = 10+i
    ...
    // 设置不同的头——错题集
    func setSpecificView_ErrorExamPaper_OnView(_ view:UIView) {
        
        // 每日一练
        let dailyBtn = UIButton(frame: CGRect(x: 20, y: 0, width: (WIDTH-60)/2.0, height: 44))
        view.addSubview(dailyBtn)
        
        let dailyChoice = UIImageView(frame: CGRect(x: 0, y: 16, width: 12, height: 12))
        dailyChoice.image = UIImage(named: "ic_choice.png")
        dailyBtn.addSubview(dailyChoice)
        let dailyLab = UILabel(frame: CGRect(x: 24, y: 12.5, width: 40, height: 17))
        dailyLab.font = UIFont.systemFont(ofSize: 18)
        dailyLab.text = "每日一练"
        dailyLab.sizeToFit()
        ...
        if btn.tag == 1 {
            ...
            if uid==nil {
                ...
                let param = [
                    ...
                    "refid":examInfo.questionid as AnyObject,
                    "type":"1" as AnyObject,
                    "userid":uid! as AnyObject,
                    "title":examInfo.post_title as AnyObject,
                    "description":examInfo.post_description as AnyObject
                ] as [String:AnyObject]
