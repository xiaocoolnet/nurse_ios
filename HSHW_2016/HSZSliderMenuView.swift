...
class HSZSliderMenuView: UIView {
 ...
 private func setViews() {
    directButton.frame = CGRect(x: frame.height/2-9, y: frame.height/2-9, width: 18, height: 18 )
    ...
    let directView = UIView(frame: CGRectMake(frame.width - frame.height,0,frame.height,frame.height))
    directView.layer.borderColor = UIColor.grayColor().CGColor
    directView.layer.borderWidth = 1
    
    scrollView.frame = CGRect(x: 0, y: 0, width:frame.width - frame.height, height: frame.height)
